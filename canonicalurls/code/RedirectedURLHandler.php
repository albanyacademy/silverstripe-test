 ...
 */
class CanonicalURLHandler extends Extension {
	...
	public function onBeforeHTTPError404($request) {
        ...
        // TODO: Get rid of the querystring stuff./*
		$SQL_base = Convert::raw2sql(rtrim($base, '/'));

		$potentials = DataObject::get("CanonicalURL", "\"File\" = '/" . $SQL_base . "'", "\"FromQuerystring\" ASC");
		$listPotentials = new ArrayList; 
		foreach  ($potentials as $potential){ 
			$listPotentials->push($potential);
		}
		
		// Find any matching File elements terminating in a wildcard
        // TODO we can probably eliminate the need for wildcards. this will strictly be a 1:1 operation. /*
		$baseparts = explode('/', $base); 
		for ($pos = count($baseparts) - 1; $pos >= 0; $pos--){
			$basestr = implode('/', array_slice($baseparts, 0, $pos));
			$basepart = Convert::raw2sql($basestr . '/*');
			$basepots = DataObject::get("RedirectedURL", "\"File\" = '/" . $basepart . "'", "\"FromQuerystring\" ASC");
		...
		// this URL may have
        // TODO: get rid of all the querystring stuff */
		if($listPotentials) {
        ...
        // TODO: Here's where the canonical script should go!!!! */
		if($matched) {
